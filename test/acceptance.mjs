...
ok('bar is 6px until the last tenth', fillH===6, `${fillH}px`)
const trackW = await p.locator('.timerbar__track').evaluate(e=>e.getBoundingClientRect().width)
ok('spent track spans the full width', Math.round(trackW)===375, `${trackW}px`)
ok('three quarter marks below the gauge', (await p.locator('.timerbar__ticks span').count())===3)
...
const purpleOnScreen = await p.evaluate(()=>[...document.querySelectorAll('main *')].some(e=>getComputedStyle(e).backgroundColor==='rgb(34, 211, 238)'))
ok('cyan and lime never on screen together', !purpleOnScreen)
...
ok('navy ground, dark only', bg==='rgb(10, 16, 23)', bg)
